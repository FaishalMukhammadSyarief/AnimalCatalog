...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.topAppbar.setOnMenuItemClickListener {
            when (it.itemId){
                R.id.about_page -> startActivity(Intent(this, DetailActivity::class.java))
            }
            true
        }

        val animalAdapter = AnimalAdapter(animalList){ toDetail(it) }
        binding.rvAnimal.adapter = animalAdapter
    }

    private fun toDetail(data: Animal){
